...
import ua.frist008.action.record.data.network.record.entity.Converter
import ua.frist008.action.record.data.network.record.entity.RecordDTO
import ua.frist008.action.record.data.network.record.entity.RecordDataType
...
class RecordNetworkSourceImpl @Inject constructor(
    ...
    override suspend fun connect(dto: DeviceDTO): Unit = withContext(Dispatchers.IO) {
        getOrCreateClientSocket(dto).use {
            DatagramSocket(SERVER_PORT, SERVER_INET_ADDRESS).use { datagramSocket ->
                ...
                try {
                    val datagramBuffer = ByteArray(NetworkConst.BYTES_IN_MB)
    ...
    private suspend fun updateData(data: ByteArray) {
        ...
        val data2And3 = ((data[2].toInt() shl ByteConst.BITS_1) and NetworkConst.FF00) or
            (data[3].toInt() and ByteConst.MAX_UBYTE)

        if (data[0] == NetworkConst.KEY_0 &&
            data[1] == NetworkConst.KEY_1 &&
            ...
            var i = 15
            val q = ByteArray(NetworkConst.BYTES_IN_KB)
